  ...
  return (
    <>
      <div className="category-row">
        {categories.map((category) => <button key={category} className={activeCategory === category ? 'active' : ''} onClick={() => onCategoryChange(category)}>{category}</button>)}
      </div>
      <div className="product-grid">
        {visibleProducts.map((product) => <article className="product-card" key={product.id}>
          <div className={`product-art ${product.tone}`}>
            <img className="product-image" src={product.image} alt={`${product.name} product design`} />
            <div className="product-actions">
              <button className="quick-add" type="button" onClick={() => onAddToCart(product)}>Add to bag <Plus size={16} /></button>
              <button className="see-more" type="button">See more <ArrowUpRight size={16} /></button>
            </div>
